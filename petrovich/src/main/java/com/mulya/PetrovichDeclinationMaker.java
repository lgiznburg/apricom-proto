...
public class PetrovichDeclinationMaker {
	...
	private PetrovichDeclinationMaker(String pathToRulesFile) throws IOException {
		//rootRulesBean = JSON.std.beanFrom(RootBean.class, new String(Files.readAllBytes(Paths.get(pathToRulesFile)), "UTF-8"));
		InputStream is = PetrovichDeclinationMaker.class.getClassLoader().getResourceAsStream(pathToRulesFile);
		rootRulesBean = JSON.std.beanFrom(RootBean.class, new String(IOUtils.toString(is, "UTF-8")));
	}
	...
	public class GenderCurryedMaker {
	...
	public class GenderAndNamePartCurryedMaker {
